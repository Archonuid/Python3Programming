...
# each of which iterates over the list several_things, but each of those 2 for loops should have a 
# different result.
...
# assigns it to sum_val (an integer). (You should use the .split("+") function to split by "+" and 
...
# and assigns it to avg_temp. 
# Do not hard code your answer (i.e., make your code compute both the sum or the number of items in 
...
# Try setting .speed(10) for the turtle to draw fast, or .speed(0) for it to draw super fast with 
# no animation.)
import turtle

archon = turtle.Turtle() 
